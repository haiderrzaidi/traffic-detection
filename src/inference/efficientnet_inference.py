...
from src.models.classification.efficientnet.efficientnet_model import EfficientNetModel
from src.configs.model_config import EFFICIENTNET_CONFIG, DATASET_CONFIG

def parse_args():
    parser = argparse.ArgumentParser(description='Run inference with trained EfficientNet model')
...
def main():
    ...
    if 'state_dict' in checkpoint:
        ...
        for k, v in state_dict.items():
            if k.startswith('model.'):
                # Remove 'model.' prefix and add it back for the model's state dict
                new_key = 'model.' + k[6:]
                ...
                new_key = 'model.' + k
                new_state_dict[new_key] = v
        print(f"New state dict keys: {list(new_state_dict.keys())[:5]}...")
        # Load the state dict into the model
        model.load_state_dict(new_state_dict)
    else:
        print("No state_dict found in checkpoint, loading directly")
    ...
    # Print results
    class_names = ['No Accident', 'Minor Accident', 'Major Accident']  # Updated class names
    print(f"\nPrediction: {class_names[prediction]}")
    print(f"Confidence: {confidence:.2%}")
    
    # Save visualization with model name
    output_path = os.path.join(args.output_dir, 'efficientnet_prediction.jpg')
    frame = frames[len(frames)//2]  # Use middle frame for visualization
    
    # Add prediction text to frame
    text = f"EfficientNet Model: {class_names[prediction]} ({confidence:.2%})"
    cv2.putText(
        frame, text, (10, 30),
