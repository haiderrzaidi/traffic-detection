...
def load_model(checkpoint_path: str) -> GRUModel:
    """Load the trained GRU model from checkpoint."""
    ...
    if not os.path.exists(checkpoint_path):
        ...
        alt_path = os.path.join(project_root, 'checkpoints', 'gru', os.path.basename(checkpoint_path))
...
def main():
    parser = argparse.ArgumentParser(description='Run inference on a video using GRU model')
    parser.add_argument('--video_path', type=str, required=True, help='Path to input video')
    parser.add_argument('--checkpoint_path', type=str, required=True, help='Path to model checkpoint')
    parser.add_argument('--output_path', type=str, default='output_gru.mp4', help='Path to save output video')
    ...
    if args.visualize or args.frames_dir:
        ...
        frames_dir = os.path.join(project_root, 'temp_frames_gru')
        if args.frames_dir:
