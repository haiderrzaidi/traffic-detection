...
def load_model(checkpoint_path: str) -> EfficientNetModel:
    ...
    if not os.path.exists(checkpoint_path):
        ...
        alt_path = os.path.join(project_root, 'checkpoints', 'efficientnet', os.path.basename(checkpoint_path))
    ...
    model = EfficientNetModel.load_from_checkpoint(checkpoint_path)
    model.eval()
    return model

def main():
    parser = argparse.ArgumentParser(description='Run inference on a video using EfficientNet model')
    parser.add_argument('--video_path', type=str, required=True, help='Path to input video')
    parser.add_argument('--checkpoint_path', type=str, required=True, help='Path to model checkpoint')
    parser.add_argument('--output_path', type=str, default='output_efficientnet.mp4', help='Path to save output video')
    ...
    if args.visualize or args.frames_dir:
        ...
        frames_dir = os.path.join(project_root, 'temp_frames_efficientnet')
        if args.frames_dir:
